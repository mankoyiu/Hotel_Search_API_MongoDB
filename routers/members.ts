...
const verifyAdmin = async (ctx: Context, next: any) => {
    try {
        ...
        const member = await db.find('members', { 
        ...
        if (member.length === 0) {
            ctx.status = 403;
            ctx.body = { msg: 'Forbidden: Member not found' };
            return;
        }

        if (member[0].role !== 0) {  // Explicit role check
            ctx.status = 403;
...
// Modified authMember to check against 'users' collection
const authMember = async (ctx: Context, next: any) => {
    try {
        ...
        // Verify user exists in users collection and has role 2 (member)
