...
const authAgency = async (ctx: Context) => {
    try {
        ...
        const { modifiedCount } = await db.update('users', { username: ctx.state.user.username }, { token });
...
// Routes
router.post('/', bodyParser(), basicAuth, verifyAdmin, newAgency);
router.put('/', bodyParser(), basicAuth, verifyAdmin, updateAgency);
...
router.get('/auth', basicAuth, authAgency);

// Get user's profile photo by username
