...
// Get all photos for the authenticated agency
...
router.get('/photos/:id', async (ctx: Context) => {
    try {
        ...
        // Verify the user has access to this photo
        if (file.metadata?.uploadedBy !== ctx.state.user?.username) {
            ctx.status = 403;
        ...
        ctx.status = 500;
        ctx.body = { msg: 'Failed to stream photo' };
    }
});
